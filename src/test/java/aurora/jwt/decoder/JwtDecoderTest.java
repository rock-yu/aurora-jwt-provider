...
class JwtDecoderTest {
    ...
    void shouldVerifyValidJwt() {
        ...
        SecurityContext securityContext = jwtDecoder.decodeJwt(jwt, LocalDate.of(2022, 5, 1).atStartOfDay(ZoneId.systemDefault()).toInstant());
    ...
    void shouldRaiseExceptionForInvalidJwt() {
        ...
        try {
            jwtDecoder.decodeJwt(invalidJwtFormat, Instant.now());
    ...
    void shouldRaiseExceptionForValidButNotVerifiedJwt() {
        ...
        try {
            jwtDecoder.decodeJwt(jwt, Instant.now());
        } catch (JwtVerificationException e) {
